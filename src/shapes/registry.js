...
import { drehknopf }      from './griffe.js'
import { wandhalterung, kabelklemme } from './halterungen.js'
import { endkappe, moebelgleiter }    from './abdeckungen2.js'
import { scharnier }      from './scharnier.js'
...
function ring(oR, iR = 0) {
  const s = new THREE.Shape()
  s.absarc(0, 0, oR, 0, Math.PI * 2, false)
  if (iR > 0) { const h = new THREE.Path(); h.absarc(0, 0, iR, 0, Math.PI * 2, true); s.holes.push(h) }
  return s
}

function extrUp(shape, h) {
  const g = new THREE.ExtrudeGeometry(shape, { depth: h, bevelEnabled: false, curveSegments: 64 })
  g.rotateX(-Math.PI / 2)
  return g
}

function hexShp(r, hR = 0) {
  const s = new THREE.Shape()
  for (let i = 0; i < 6; i++) {
  ...
  s.closePath()
  if (hR > 0) { const h = new THREE.Path(); h.absarc(0, 0, hR, 0, Math.PI * 2, true); s.holes.push(h) }
  return s
}
...
export const SHAPES = {
  ...
  // ── Grundformen ─────────────────────────────────────────────────────────────
  box: {
    label: 'Quader / Würfel', shortLabel: 'Quader', icon: '⬜', group: 'Grundformen',
    fields: [
  ...
  cylinder: {
    label: 'Zylinder (massiv)', shortLabel: 'Zylinder', icon: '⬤', group: 'Grundformen',
    fields: [
  ...
  sphere: {
    label: 'Kugel', shortLabel: 'Kugel', icon: '🔵', group: 'Grundformen',
  ...
  kegel: {
    label: 'Kegelstumpf / Konus', shortLabel: 'Konus', icon: '▲', group: 'Grundformen',
    fields: [
    ...
    calcVolume({ diameter_bottom, diameter_top, height }) {
      const r1=diameter_bottom/2,r2=diameter_top/2; return (Math.PI/3)*(r1*r1+r1*r2+r2*r2)*height
    },
  },

  // ── Rohre & Hülsen ──────────────────────────────────────────────────────────
  hollow_cylinder: {
    label: 'Hohlzylinder / Distanzscheibe', shortLabel: 'Distanzscheibe', icon: '⭕', group: 'Rohre & Hülsen',
    fields: [
    ...
    buildGeometry({ outer_diameter, inner_diameter, height }) {
      return { geometry: extrUp(ring(outer_diameter/2, Math.min(inner_diameter/2,outer_diameter/2-0.5)), height), positionY: 0, cameraR: Math.max(outer_diameter,height)*4 }
    },
  ...
  rohr: {
    label: 'Rohr / Langstutzen', shortLabel: 'Rohr', icon: '≡', group: 'Rohre & Hülsen',
    fields: [
    ...
    buildGeometry({ outer_diameter, inner_diameter, height }) {
      return { geometry: extrUp(ring(outer_diameter/2, Math.min(inner_diameter/2,outer_diameter/2-0.5)), height), positionY: 0, cameraR: Math.max(outer_diameter,height)*3 }
    },
    calcVolume({ outer_diameter, inner_diameter, height }) {
      return Math.PI*(Math.pow(outer_diameter/2,2)-Math.pow(inner_diameter/2,2))*height
    },
  ...
  // ── Verbindungen ─────────────────────────────────────────────────────────────
  flansch: {
    label: 'Flansch', shortLabel: 'Flansch', icon: '⊥', group: 'Verbindungen',
    fields: [
    ...
    buildGeometry({ flange_diameter, pipe_diameter, wall_thickness, flange_thickness, pipe_height }) {
      ...
      const fg=extrUp(ring(fR,pIR),flange_thickness), pg=extrUp(ring(pOR,pIR),pipe_height)
      pg.translate(0,flange_thickness,0)
      return { geometry: merge([fg,pg]), positionY: 0, cameraR: Math.max(flange_diameter,pipe_height+flange_thickness)*2.8 }
    },
  ...
  sechskant: {
    label: 'Sechskant-Bolzen / Standoff', shortLabel: 'Sechskant', icon: '⬡', group: 'Verbindungen',
    fields: [
    ...
    buildGeometry({ width_across_flats, height }) {
      ...
      return { geometry: extrUp(hexShp(r), height), positionY: 0, cameraR: Math.max(width_across_flats,height)*3.5 }
    },
    calcVolume({ width_across_flats, height }) { return (Math.sqrt(3)/2)*Math.pow(width_across_flats,2)*height },
  },

  sechskant_mutter: {
    label: 'Sechskantmutter (mit Loch)', shortLabel: 'Mutter', icon: '⬡', group: 'Verbindungen',
    fields: [
      { id: 'width_across_flats', label: 'Schlüsselweite', min: 2,  max: 200, default: 13  },
    ...
    buildGeometry({ width_across_flats, thread_diameter, height }) {
      const r=(width_across_flats/2)/Math.cos(Math.PI/6)
      return { geometry: extrUp(hexShp(r,Math.min(thread_diameter/2,r-0.5)), height), positionY: 0, cameraR: Math.max(width_across_flats,height)*4 }
    },
  ...
  // ── Abdeckungen ──────────────────────────────────────────────────────────────
  abdeckkappe: {
    label: 'Abdeckkappe (Deckel mit Lippe)', shortLabel: 'Abdeckkappe', icon: '⊏', group: 'Abdeckungen',
    fields: [
    ...
    buildGeometry({ outer_diameter, wall_thickness, lip_height, top_thickness }) {
      ...
      const dg=extrUp(ring(oR),top_thickness), lg=extrUp(ring(oR,iR),lip_height)
      lg.translate(0,top_thickness,0)
      return { geometry: merge([dg,lg]), positionY: 0, cameraR: Math.max(outer_diameter,lip_height+top_thickness)*3 }
    },
